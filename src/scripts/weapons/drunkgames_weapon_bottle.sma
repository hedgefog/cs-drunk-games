...
#define PLUGIN "[Drunk Games] Weapon Bottle"
...
public plugin_precache() {
    precache_generic(DRUNKGAME_WEAPON_BOTTLE_HUD_TXT);
    precache_model(DRUNKGAME_MODEL_WEAPON_BOTTLE_V);
    precache_model(DRUNKGAME_MODEL_WEAPON_BOTTLE_P);
    precache_model(DRUNKGAME_MODEL_WEAPON_BOTTLE_W);
    precache_sound(DRUNKGAME_SOUND_BOTTLE_THROW);
}

public plugin_init() {
    register_plugin(PLUGIN, VERSION, AUTHOR);

    g_iCwHandler = CW_Register(DRUNKGAME_WEAPON_BOTTLE, CSW_DEAGLE, 1, _, _, _, _, 0, 1, _, "skull", CWF_NoBulletSmoke);
...
public @Weapon_Idle(this) {
    ...
    if (get_member(this, m_flStartThrow)) {
        ThrowGrenade(this);
        emit_sound(pPlayer, CHAN_BODY, DRUNKGAME_SOUND_BOTTLE_THROW, 1.0, ATTN_NORM, 0, PITCH_NORM);
        return;
    }
    ...
    if (g_bPlayerRedeploy[pPlayer]) {
        ...
        CW_PlayAnimation(this, 0, 61.0 / 30.0);
    }
}

public @Weapon_Deploy(this) {
    new pPlayer = CW_GetPlayer(this);
    CW_DefaultDeploy(this, DRUNKGAME_MODEL_WEAPON_BOTTLE_V, DRUNKGAME_MODEL_WEAPON_BOTTLE_P, 3, "grenade");
...
public @Weapon_PrimaryAttack(this) {
    if (!get_member(this, m_flStartThrow)) {
        ...
        CW_PlayAnimation(this, 1, 19.0 / 30.0);
    } else {
        new pPlayer = CW_GetPlayer(this);
...
public @Weapon_WeaponBoxSpawn(this, pWeaponBox) {
    engfunc(EngFunc_SetModel, pWeaponBox, DRUNKGAME_MODEL_WEAPON_BOTTLE_W);
}
...
ThrowGrenade(this) {
    ...
    new pBottle = ShootTimed(pPlayer, vecSrc, vecThrow);
    set_pev(pBottle, pev_angles, vecThrowAngle);

    CW_PlayAnimation(this, 2, 16.0 / 30.0);
...
ShootTimed(pOwner, const Float:vecStart[3], const Float:vecVelocity[3]) {
    new pBottle = CE_Create("drunkgames_bottle", vecStart);
    set_pev(pBottle, pev_owner, pOwner);
    dllfunc(DLLFunc_Spawn, pBottle);
    engfunc(EngFunc_SetOrigin, pBottle, vecStart);
    set_pev(pBottle, pev_velocity, vecVelocity);

    static Float:vecAngles[3];
    vector_to_angle(vecVelocity, vecAngles);
    set_pev(pBottle, pev_angles, vecAngles);
