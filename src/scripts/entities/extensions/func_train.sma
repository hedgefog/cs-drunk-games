...
#define ENTITY_NAME ENTITY(Train)
#define MEMBER ENTITY_MEMBER<Train>
...
public plugin_precache() {
  ...
  CE_ImplementClassMethod(ENTITY_NAME, CE_Method_Activate, "@Entity_Activate");
  CE_ImplementClassMethod(ENTITY_NAME, CE_Method_Restart, "@Entity_Restart");
}

public plugin_init() {
  register_plugin(ENTITY_EXTENSION_PLUGIN(Train), DRUNKGAMES_VERSION, "Hedgehog Fog");
}
...
@Entity_Spawn(const this) {
  ...
  CE_SetMemberString(this, MEMBER(szFirstTarget), szTarget);

  CE_SetMember(this, MEMBER(flSpeed), flSpeed);
...
@Entity_Restart(const this) {
  static szFirstTarget[32]; CE_GetMemberString(this, MEMBER(szFirstTarget), szFirstTarget, charsmax(szFirstTarget));
  ...
  set_pev(this, pev_speed, Float:CE_GetMember(this, MEMBER(flSpeed)));
