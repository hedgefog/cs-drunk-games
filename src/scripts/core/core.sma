...
public plugin_precache() {
  ...
  hook_cvar_change(create_cvar(CVAR("version"), DRUNKGAMES_VERSION, FCVAR_SERVER), "CvarHook_Version");
}

public plugin_init() {
  register_plugin(DRUNKGAMES_TITLE, DRUNKGAMES_VERSION, "Hedgehog Fog");
  ...
  g_pfwConfigLoaded = CreateMultiForward("DrunkGames_OnConfigLoaded", ET_IGNORE);
}
...
public plugin_cfg() {
  ...
  server_cmd("exec %s/drunkgames/%s.cfg", szConfigDir, szMapName);
  server_exec();
...
public plugin_natives() {
  register_library(DRUNKGAMES_LIBRARY);
}

/*--------------------------------[ Commands ]--------------------------------*/

public Command_Radio(const pPlayer) {
...
public CvarHook_Version(const pCvar) {
  set_pcvar_string(pCvar, DRUNKGAMES_VERSION);
}

public FMHook_GetGameDescription() {
  static szGameName[32];
  format(szGameName, charsmax(szGameName), "%s %s", DRUNKGAMES_TITLE, DRUNKGAMES_VERSION);
